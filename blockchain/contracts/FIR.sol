...
contract FIR {
    ...
    function entry(
        ...
        string memory _policeDepartment
    ) public onlyOwner {
        fir[criminalCount] = dataPerson(
            _accusedName,
            _victimName,
            _date,
            _time,
    ...
    // retrieve
    function retrieve(uint256 _entryno)
        public
        view
        returns (
            string memory _victimName,
        ...
        )
    {
        for (uint256 i = 0; i < criminalCount; i++) {
            if (
                keccak256(abi.encodePacked(fir[_entryno].accusedName)) ==
                keccak256(abi.encodePacked(fir[i].accusedName))
            ) {
                return (
                    fir[i].victimName,
                    fir[i].date,
                    ...
                    fir[i].incident,
                    fir[i].witnessNames,
                    fir[i].descriptionOfTheAccused,
                    fir[i].policeName,
                    fir[i].policeRank,
                    fir[i].policeDepartment
                );
            }
        }
    }
    ...
    function checkDataPerson(uint256 _entryno)
        public
        view
        onlyOwner
        returns (bool flag)
    {
        for (uint256 i = 0; i < DataCriminal.length; i++) {
            flag =
                keccak256(abi.encodePacked(DataCriminal[i])) ==
                keccak256(abi.encodePacked(fir[_entryno].accusedName));
            if (flag) {
                break;
            }
    ...
    // list of criminals
    function checkDataCriminal(string memory _accusedName)
        public
        view
        returns (bool flag)
    {
        for (uint256 i = 0; i < DataCriminal.length; i++) {
            flag =
                keccak256(abi.encodePacked(DataCriminal[i])) ==
                keccak256(abi.encodePacked(_accusedName));
            if (flag) {
                break;
            }
        }
